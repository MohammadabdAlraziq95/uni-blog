...
export class UPostDetailComponent implements OnInit {
  ...
  getPublicPostsFromProfileId(profileid) {
    ...
    this.acrud.getPublicPostsFromProfileId(profileid).subscribe((d) => {
      ...
      this.getComment(
        this.publicpostOfSingleUser.uid,
        this.publicpostOfSingleUser.title,
        this.publicpostOfSingleUser.desc
      );
      this.acrud.getPostDetailForLike(
        this.post_userid,
        this.posttitle,
        this.postdesc
      );
  ...
  getFeaturedPost() {
    ...
    this.acrud.getFeaturedPost().then((d: any) => {
      ...
      this.acrud
        .getPublicProfile(this.publicpostOfSingleUser.uname)
        .subscribe((d) => {
          let x = this.acrud.seprate(d);
          this.ProfileImgUrl = x[0]?.imgurl;
        });
    ...
    this.acrud.getAllPost().then(
      (x: any) => {
        ...
        if (this.unauthpost?.uid) {
          ...
          this.getComment(
            this.post_userid,
            this.unauthpost.title,
            this.unauthpost.desc
          );
        } else {
  ...
  getUauthPublicPost() {
    this.isUnauth = true;
    this.isFetching = true;
    this.cd.get_public_post().subscribe(
      (result) => {
        this.unauthpostss = result.map(
          (e) => {
            return {
  ...
  getComment(postid, title, desc) {
    let CommentKeyPromise = this.acrud
      .getCommentKey(postid, title, desc)
      .then((d) => {
    ...
    if (CommentKeyPromise) {
      CommentKeyPromise.then((key) => {
        this.acrud
          .getCommentDataFromKey(postid, key)
          .subscribe((commentData: Comment) => {
            this.Comment_Data = this.acrud.seprate(commentData);

            if (commentData) {
              this.Comment_Data.sort(
                (a, b) =>
                  new Date(b.commentOn).getTime() -
                  new Date(a.commentOn).getTime()
              );
            }

            for (let i in this.Comment_Data) {
              this.acrud
                .getProfileFromUid(this.Comment_Data[i].commentByUserId)
                .subscribe((data) => {
  ...
  getProfileFromUid(postuserid) {
    this.acrud.getProfileFromUid(postuserid).subscribe((data) => {
  ...
  getAuthPublicPost() {
    ...
    this.puSub = this.acrud.pu.subscribe(
      (d) => {
        if (d) {
          ...
          this.getComment(
            this.public_post[id].uid,
            this.public_post[id].title,
            this.public_post[id].desc
          );
  ...
  getAuthAllPost() {
    ...
    this.acrud.getAllData().subscribe(
      (data) => {
        this.isFetching = false;
        let x1 = data[0];
        let x2 = data[1];
        ...
        this.getComment(
          this.allPost[id].uid,
          this.allPost[id].title,
          this.allPost[id].desc
        );
      },
  ...
  onSubmit(value: Comment) {
    if (this.isAuthenticated) {
      ...
      this.toastr.error(
        "You have to login so you can comment and add new questions",
        "Error",
        {
          timeOut: 2e4,
        }
  ...
  OnDelete() {
    if (this.xyz[2] == "allpost") {
      ...
      this.acrud.deletPostEvent(this.allPost[this.id], this.id).then((d) => {
    ...
    if (this.xyz[2] == "public") {
      this.acrud.passParams(this.xyz[2], this.id);
      this.acrud
        .deletPostEvent(this.public_post[this.id], this.id)
        .then((d) => {
          this.acrud.showSuccessDelete();
        });
    }
    if (this.xyz[2] == "private") {
      this.acrud.passParams(this.xyz[2], this.id);
      this.acrud
        .deletPostEvent(this.private_post[this.id], this.id)
        .then((d) => {
          this.acrud.showSuccessDelete();
        });
    }
